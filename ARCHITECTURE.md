...
| CRDT | `src/crdt/doc.ts` | Document lifecycle, Y.Doc management, IndexedDB persistence |
| CRDT | `src/crdt/sync.ts` | Realtime broadcast sync, checkpoint persistence |
| CRDT | `src/crdt/awareness.ts` | User presence via Awareness protocol |
| CRDT | `src/crdt/offline.ts` | Explicit offline caching, storage stats, text extraction |
...
| **CRDT collaborative editing** | Yjs documents with IndexedDB persistence, Supabase Broadcast sync, awareness presence, offline cache |
...
**Files**: `src/crdt/doc.ts`, `src/crdt/sync.ts`, `src/crdt/awareness.ts`, `src/crdt/offline.ts`

The engine provides a complete Yjs CRDT integration layer for collaborative editing. Applications use this module for rich-text or structured-data documents that need real-time multi-user editing with automatic conflict resolution at the character level — a fundamentally different approach from the row-level sync engine described in sections 3–5.
...
|                                                                   |
|  +-------------------+    +---------------------+                 |
|  | Y.Doc             |    | IndexeddbPersistence |                |
|  | (in-memory CRDT)  |--->| (y-indexeddb)        |                |
|  |                   |    | Local durability     |                |
|  +--------+----------+    +---------------------+                 |
|           |                                                       |
|           | doc.on('update')                                      |
...
|  | crdt_{prefix}_{id}|    | (WebSocket)         |                |
|  |                   |    |                     |                 |
|  | Echo suppression  |    | Binary updates      |                |
|  | via device_id     |    | as base64           |                |
|  +-------------------+    +---------------------+                 |
|           |                                                       |
|           | Checkpoint (debounced 5s)                             |
|           v                                                       |
|  +-------------------+                                            |
|  | note_content      |    Supabase table                         |
|  | (yjs_state col)   |    Full doc state as base64               |
|  +-------------------+                                            |
|                                                                   |
|  AWARENESS (separate channel):                                    |
|  +-------------------+    +---------------------+                 |
|  | Awareness         |<-->| Broadcast Channel   |                |
|  | (y-protocols)     |    | awareness_{prefix}_ |                |
|  | User presence,    |    | {id}                |                |
|  | cursor positions  |    |                     |                |
|  +-------------------+    +---------------------+                 |
+------------------------------------------------------------------+
...
initCrdtDoc(noteId)
  |
  +---> Create Y.Doc (in-memory CRDT)
  +---> Attach IndexeddbPersistence (local durability)
  +---> Store in activeDocs map
  +---> Return Y.Doc
  |
  v
waitForCrdtSync(noteId)
  |
  +---> Waits for IndexedDB to finish loading
  |     (resolves immediately if already synced)
  |
  v
loadCrdtFromRemote(noteId)
  |
  +---> Fetch yjs_state from note_content table
  +---> Y.applyUpdate(doc, state, 'remote-load')
  |
  v
connectCrdtRealtime(noteId)
  |
  +---> Open broadcast channel: crdt_{prefix}_{noteId}
  +---> Attach doc.on('update') listener
  +---> On SUBSCRIBED:
  |       - Send accumulated updates (if any)
  |       - Request state from peers (reconnect sync)
  |
  v
(user edits document)
  |
  +---> Y.Doc updates locally (instant)
  +---> Update broadcast to other clients
  +---> Checkpoint scheduled (5s debounce)
  |
  v
App navigates away
  |
  v
disconnectCrdtRealtime(noteId)
  |
  +---> Save final checkpoint
  +---> Close broadcast channel
  |
  v
destroyCrdtDoc(noteId)
  |
  +---> Run cleanup callbacks
  +---> Close IndexeddbPersistence
  +---> Destroy Y.Doc
  +---> Remove from activeDocs map
```

### 13.3 Realtime Sync Protocol

The CRDT realtime sync uses Supabase Broadcast (not postgres_changes). Four message types flow through the channel:

| Message Type | Direction | Purpose |
|-------------|-----------|---------|
| `update` | Bidirectional | Incremental Yjs update (binary, base64-encoded) |
| `state-vector-request` | Outbound on connect | Requests full state from peers for reconnect sync |
| `state-vector-response` | Response to request | Full document state sent to reconnecting peer |
| `awareness` | Bidirectional | User presence state (separate channel) |

**Echo suppression**: Each message carries a `deviceId`. Updates originating from the local device are ignored. The channel is also configured with `self: false` as a primary filter.

**Origin tracking**: Y.Doc updates are tagged with origin strings (`'remote-broadcast'`, `'remote-load'`, `'remote-state-sync'`) to prevent re-broadcasting received updates.

**Checkpoint persistence**: After local edits, a checkpoint save is debounced (5 seconds). The full Y.Doc state is encoded and upserted to the `note_content` Supabase table. Checkpoints are also saved on page visibility hidden (user switching tabs or closing the app).

### 13.4 Awareness (Presence)

Awareness runs on a **separate broadcast channel** (`awareness_{prefix}_{docId}`) from CRDT sync. This separation allows presence updates to flow independently of document edits.

```
initAwareness(docId, userInfo)
  |
  +---> Create Awareness(doc) instance
  +---> Set local state: { user: { name, color, role, scope, cursor } }
  +---> Open awareness broadcast channel
  +---> On local awareness change → broadcast to peers
  +---> On remote awareness update → apply to local awareness
  +---> On window beforeunload → remove local awareness state
```

User roles:
- `'owner'` — the note creator/owner
- `'shared'` — a user accessing via share link
...
The offline cache is a separate system from the active `y-indexeddb` persistence. While `y-indexeddb` automatically persists documents that are currently open, the offline cache explicitly stores documents the user has marked for offline access.

```
+-------------------+     +-------------------+     +------------------+
| Active Doc        |     | Offline Cache     |     | Supabase         |
| (y-indexeddb)     |     | ({prefix}_offline |     | (note_content)   |
| Auto-persisted    |     | _crdt)            |     |                  |
| while doc is open |     | Explicit download |     | Checkpoint store |
+-------------------+     +-------------------+     +------------------+
        |                         |                         |
        +--- cacheCrdtForOffline -+-- (prefers live doc) ---+
        |                         |                         |
        +-- ensureCrdtOfflineData +-- (bridges to active) --+
        |                         |                         |
        +--- extractCrdtText -----+-- (search indexing) ----+
```

`extractCrdtText()` reads blocks from the Y.Doc (active → offline cache → remote), strips HTML, and returns plain text for client-side search indexing.

### 13.6 Key Design Decisions

| Decision | Rationale |
|----------|-----------|
| Supabase Broadcast (not postgres_changes) | Binary Yjs updates cannot be represented as row changes. Broadcast provides raw WebSocket message passing. |
| Separate channels for sync vs awareness | Presence updates are high-frequency and should not interfere with document sync. |
| Checkpoint to `note_content` table | Full Yjs state is stored as base64 in a single column. This enables loading the complete document on a new device without replaying all incremental updates. |
| Apps never import `yjs` directly | The CRDT module re-exports all needed Yjs types. This allows the engine to manage Yjs lifecycle, persistence, and sync without apps creating unmanaged Y.Doc instances. |
| `y-indexeddb` for active docs | Provides automatic local durability for open documents. If the browser crashes, the document state is preserved. |
| Separate offline cache | Active persistence only works while a document is open. The offline cache stores documents explicitly downloaded for offline reading. |
