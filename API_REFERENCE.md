...
| `@prabhask5/stellar-engine/crdt` | CRDT document lifecycle, realtime sync, awareness, offline cache |
...
Poll for device verification completion. Checks if the current session has been confirmed (e.g., by the user clicking the email confirmation link in another tab). Used for cross-tab verification where the confirm page verifies the OTP and this tab polls to detect when it completes.

```ts
async function pollDeviceVerification(): Promise<boolean>
```

**Returns:** `true` if the device has been verified (user session is valid), `false` otherwise.

### `fetchRemoteGateConfig()`

Fetch the single-user gate configuration from the server via an RPC call (`get_extension_config`). Used by browser extensions or secondary apps to retrieve the gate type, code length, email, and profile for an existing account without having local IndexedDB state.
...
**Returns:** The remote gate configuration, or `null` if unavailable or the RPC fails.

### `resetSingleUserRemote()`

Perform a full server-side reset of the single-user account via the `reset_single_user` RPC. Deletes all user data from the server, signs out of Supabase, and clears local state (IndexedDB config + offline credentials). Used for complete account deletion.

```ts
async function resetSingleUserRemote(): Promise<{ error: string | null }>
```

**Returns:** `{ error: null }` on success, or `{ error: string }` if the RPC fails.
...
Trust the current device for the authenticated user. Unlike `trustCurrentDevice` which requires a userId parameter, this function reads the user from the current Supabase session. Used internally after OTP verification completes.
...
| `detectActionType` | `(eventType, fields, valueDelta?) => RemoteActionType` | Detect the action type from an event type and changed fields |
| `hasDeferredChanges` | `(entityId, entityType) => boolean` | Check if there are deferred changes waiting for an entity |
| `clearDeferredChanges` | `(entityId, entityType) => void` | Clear deferred changes for an entity |
...
Log a debug message. Shorthand for `debug('log', ...args)`. Only outputs when debug mode is enabled (`localStorage` key `{prefix}_debug_mode` is `'true'`).
...
Log a debug warning. Shorthand for `debug('warn', ...args)`. Only outputs when debug mode is enabled.
...
Log a debug error. Shorthand for `debug('error', ...args)`. Only outputs when debug mode is enabled.
...
> **Subpath:** `@prabhask5/stellar-engine/crdt`

The CRDT module provides collaborative editing support using Yjs, a high-performance CRDT (Conflict-free Replicated Data Type) implementation. It manages Yjs document lifecycle, IndexedDB persistence, realtime sync via Supabase Broadcast, user presence/awareness, and offline caching. Applications import all Yjs-related functionality from this module — never import `yjs` or `y-indexeddb` directly.

### Yjs Re-exports

The module re-exports core Yjs types so applications don't need a direct `yjs` dependency:

```ts
import { Y, YDoc, YArray, YMap, YText } from '@prabhask5/stellar-engine/crdt';
```

| Export | Yjs Original | Description |
|--------|-------------|-------------|
| `Y` | `* as Y from 'yjs'` | Full Yjs namespace for advanced usage (`Y.encodeStateAsUpdate`, `Y.applyUpdate`, etc.) |
| `YDoc` | `Y.Doc` | Yjs document class — the root container for all shared data |
| `YArray` | `Y.Array` | Shared array type for ordered collections |
| `YMap` | `Y.Map` | Shared map type for key-value data |
| `YText` | `Y.Text` | Shared text type for collaborative text editing |
| `Awareness` | `Awareness` from `y-protocols/awareness` | Awareness protocol class for user presence |

### Document Lifecycle

#### `initCrdtDoc(docId, config?)`

Initialize a CRDT document with local IndexedDB persistence. Creates a `Y.Doc` and attaches `IndexeddbPersistence` for local durability. If the document already exists, returns the existing instance.

```ts
function initCrdtDoc(docId: string, config?: CrdtDocConfig): Y.Doc
```

| Parameter | Type | Description |
|-----------|------|-------------|
| `docId` | `string` | Unique identifier for the document (typically a note ID) |
| `config` | `CrdtDocConfig` | Optional configuration |
...
```ts
import { initCrdtDoc, waitForCrdtSync } from '@prabhask5/stellar-engine/crdt';

const doc = initCrdtDoc('note-abc-123');
await waitForCrdtSync('note-abc-123'); // Wait for IndexedDB to load
...
Get an active CRDT document by ID. Returns `undefined` if the document has not been initialized.

```ts
function getCrdtDoc(docId: string): Y.Doc | undefined
```

#### `destroyCrdtDoc(docId)`

Destroy a CRDT document and clean up all resources. Closes IndexedDB persistence, runs cleanup callbacks, removes event listeners, and removes the document from the active map.

```ts
async function destroyCrdtDoc(docId: string): Promise<void>
```

#### `waitForCrdtSync(docId)`

Wait for a document's IndexedDB persistence to finish syncing. Resolves immediately if already synced. This should be called after `initCrdtDoc()` before reading document content to ensure local data has been loaded.

```ts
async function waitForCrdtSync(docId: string): Promise<void>
```

#### `getActiveCrdtDocIds()`

Get the IDs of all currently active (initialized) CRDT documents.

```ts
function getActiveCrdtDocIds(): string[]
```
...
CRDT realtime sync uses Supabase Broadcast channels (WebSocket) to sync Yjs document updates between clients in real time. This is separate from the main sync engine's realtime subscriptions — it handles binary Yjs updates rather than row-level PostgreSQL changes.

#### `connectCrdtRealtime(docId)`

Connect a CRDT document to Supabase Realtime for live sync. Opens a broadcast channel, attaches update listeners to the Y.Doc, and handles incoming remote updates with echo suppression via device ID. On connection, requests state from any connected peers for reconnection sync. Local updates are automatically broadcast. Checkpoint saves are scheduled after edits (5-second debounce) and on page visibility hidden.

```ts
function connectCrdtRealtime(docId: string): void
```

| Parameter | Type | Description |
|-----------|------|-------------|
| `docId` | `string` | Document/note ID (must be initialized via `initCrdtDoc` first) |

**Example:**

```ts
import { initCrdtDoc, connectCrdtRealtime, loadCrdtFromRemote } from '@prabhask5/stellar-engine/crdt';

const doc = initCrdtDoc('note-abc-123');
await loadCrdtFromRemote('note-abc-123'); // Load initial state from server
connectCrdtRealtime('note-abc-123');       // Start live sync
```

#### `disconnectCrdtRealtime(docId)`

Disconnect a CRDT document from Supabase Realtime. Saves a final checkpoint, removes update listeners and visibility handler, and closes the broadcast channel.

```ts
async function disconnectCrdtRealtime(docId: string): Promise<void>
```

#### `saveCrdtCheckpoint(docId)`

Save a checkpoint of the full Yjs document state to the `note_content` Supabase table. Encodes the entire document as a base64 string and upserts it. Called automatically on a 5-second debounce after edits and when the page visibility changes to hidden.

```ts
async function saveCrdtCheckpoint(docId: string): Promise<void>
```

#### `loadCrdtFromRemote(docId)`

Load initial CRDT state from the remote `note_content` table. Fetches the `yjs_state` column and applies it to the local Y.Doc. Call this after `initCrdtDoc()` and before `connectCrdtRealtime()` to ensure the document has the latest server state.

```ts
async function loadCrdtFromRemote(docId: string): Promise<void>
```

#### `getCrdtSyncState(docId)`

Get the sync state for a connected document.

```ts
function getCrdtSyncState(docId: string): CrdtSyncState | undefined
```

**Returns:** The sync state if the document is connected to realtime, `undefined` otherwise.

#### `isCrdtRealtimeConnected(docId)`

Check if a document is currently connected to realtime.

```ts
function isCrdtRealtimeConnected(docId: string): boolean
```

### Awareness (Presence)

The awareness module manages user presence indicators using the Yjs Awareness protocol. Each client broadcasts its presence state (user info, cursor position) via a dedicated Supabase Broadcast channel, separate from the CRDT sync channel.

#### `initAwareness(docId, userInfo)`

Initialize awareness (presence) for a CRDT document. Creates an `Awareness` instance, sets the local user's state, and opens a broadcast channel for syncing presence between clients. If awareness already exists for the document, updates the user info and returns the existing instance. Cleans up on `beforeunload`.

```ts
function initAwareness(docId: string, userInfo: AwarenessUser): Awareness
```

| Parameter | Type | Description |
|-----------|------|-------------|
| `docId` | `string` | Document/note ID (must be initialized via `initCrdtDoc` first) |
| `userInfo` | `AwarenessUser` | Current user's display information |

**Returns:** `Awareness` — The Yjs Awareness instance.

**Example:**

```ts
import { initAwareness, getRemoteAwarenessUsers } from '@prabhask5/stellar-engine/crdt';

const awareness = initAwareness('note-abc-123', {
  name: 'Alice',
  color: '#ff6b6b',
  role: 'owner'
});

// Later: check who else is viewing
const remoteUsers = getRemoteAwarenessUsers('note-abc-123');
```
...
```ts
function getAwareness(docId: string): Awareness | undefined
```

**Returns:** The `Awareness` instance if initialized, `undefined` otherwise.

#### `destroyAwareness(docId)`

Destroy awareness for a document. Removes awareness states, closes the broadcast channel, and cleans up event listeners.

```ts
async function destroyAwareness(docId: string): Promise<void>
```

#### `updateAwarenessCursor(docId, cursor)`

Update the local user's cursor position in awareness. Other clients will see the cursor move in real time.

```ts
function updateAwarenessCursor(
  docId: string,
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `docId` | `string` | Document/note ID |
| `cursor` | `{ blockId: string; offset: number } \| null` | Cursor position within a block, or `null` to clear |

#### `getRemoteAwarenessUsers(docId)`

Get all remote users currently present on a document. Returns awareness state from all connected clients except the local one.

```ts
function getRemoteAwarenessUsers(docId: string): RemoteUser[]
```

**Returns:** `RemoteUser[]` — Array of remote user presence states.

### Offline Cache

The offline cache provides explicit offline caching for CRDT documents the user wants available offline, separate from the active `y-indexeddb` persistence (which only handles currently open documents). Uses a dedicated IndexedDB store (`{prefix}_offline_crdt`).

#### `cacheCrdtForOffline(docId)`

Cache a CRDT document for offline access. Serializes the current Yjs state and stores it in IndexedDB. If the document is currently active, uses its live state; otherwise fetches from Supabase.

```ts
async function cacheCrdtForOffline(docId: string): Promise<void>
```

#### `removeCrdtOfflineCache(docId)`

Remove a document from the offline cache.

```ts
async function removeCrdtOfflineCache(docId: string): Promise<void>
```

#### `isCrdtCachedOffline(docId)`

Check if a document is cached for offline access.

```ts
async function isCrdtCachedOffline(docId: string): Promise<boolean>
```

#### `loadCrdtFromOfflineCache(docId)`

Load a cached CRDT document state from offline storage. Creates a new `Y.Doc` and applies the cached state. The caller is responsible for destroying the returned doc when done.

```ts
async function loadCrdtFromOfflineCache(docId: string): Promise<Y.Doc | null>
```

**Returns:** A `Y.Doc` with the cached state, or `null` if not cached.

#### `ensureCrdtOfflineData(docId)`

Bridge offline cache to the active CRDT document. If the active document is empty but cached data exists, applies the cached state. Useful during initialization to restore content when the document hasn't synced from the server yet.

```ts
async function ensureCrdtOfflineData(docId: string): Promise<boolean>
```

**Returns:** `true` if cached data was applied, `false` otherwise.

#### `getCrdtOfflineCacheSize(docId)`

Get the byte size of a single cached note's CRDT data.

```ts
async function getCrdtOfflineCacheSize(docId: string): Promise<number>
```

**Returns:** Size in bytes, or `0` if not cached.

#### `getActiveCrdtDocSize(docId)`

Get the byte size of the currently active CRDT document's state.

```ts
function getActiveCrdtDocSize(docId: string): number
```

**Returns:** Size in bytes, or `0` if the document is not active.

#### `getOfflineCacheStats()`

Get all cached documents with their sizes and cache timestamps.

```ts
async function getOfflineCacheStats(): Promise<{
...
Get device storage estimate (IndexedDB quota and usage) via the Storage API.
...
**Returns:** Storage estimates, or `null` if the Storage API is not available.

#### `formatBytes(bytes)`

Format a byte count to a human-readable string (e.g., `"1.5 KB"`, `"2.3 MB"`).

```ts
function formatBytes(bytes: number): string
```

#### `extractCrdtText(docId)`

Extract plain text from a note's CRDT blocks. Reads block content from the active document, offline cache, or Supabase remote (in that priority order). Returns concatenated text content with HTML tags stripped. Useful for search indexing.

```ts
async function extractCrdtText(docId: string): Promise<string>
```

**Returns:** Plain text content of all blocks, or empty string if unavailable.
...
```ts
type AwarenessRole = 'owner' | 'shared';
```

#### `AwarenessScope`

```ts
type AwarenessScope = 'READ' | 'WRITE' | 'COMMENT';
```
