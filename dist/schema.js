...
export function generateTypeScript(schema, options) {
    ...
    const header = options?.header ?? '/** AUTO-GENERATED by stellar-drive — do not edit manually. */';
    lines.push(header);
    lines.push('');
            ...
            if (enumDef) {
                ...
                if (existing) {
                    ...
                    if (oldValues !== newValues) {
                        console.warn(`[stellar-drive] Enum name collision: "${enumDef.name}" is defined with different values.\n` +
                            `  First:  ${oldValues}\n` +
...
export function generateSupabaseSQL(schema, options) {
    ...
    if (includeDeviceVerification) {
        ...
        parts.push('alter table trusted_devices drop constraint if exists trusted_devices_user_id_device_id_key;');
