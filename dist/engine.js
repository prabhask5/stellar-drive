...
import { startRealtimeSubscriptions, stopRealtimeSubscriptions, onRealtimeDataUpdate, onConnectionStateChange, cleanupRealtimeTracking, isRealtimeHealthy, pauseRealtime, wasRecentlyProcessedByRealtime, suspendRealtimeForBatchPush, resumeRealtimeAfterBatchPush } from './realtime';
...
const SYNC_OPERATION_TIMEOUT_BASE_MS = 45000;
/**
 ...
 */
function getSyncOperationTimeout(pendingCount = 0) {
    const extra = Math.ceil(pendingCount / 500) * 30000;
    return Math.min(SYNC_OPERATION_TIMEOUT_BASE_MS + extra, 300000);
}
/**
 ...
 */
function handleSyncItemError(item, error) {
...
export async function runFullSync(quiet = false, skipPull = false) {
    ...
    try {
        ...
        if (shouldSuspendRealtime) {
            debugLog(`[SYNC] ${preflightItems.length} pending ops — suspending realtime to prevent egress flood`);
        ...
        try {
            ...
            if (postSyncRemaining.length > 0) {
                debugLog(`[SYNC] ${postSyncRemaining.length} item(s) remain after sync — scheduling follow-up push`);
                scheduleSyncPush();
            }
        }
        catch {
...
async function fullReconciliation() {
    ...
    try {
        ...
        const results = await withTimeout(Promise.all(config.tables.map((table) => supabase.from(table.supabaseName).select('id').or('deleted.is.null,deleted.eq.false'))), getSyncOperationTimeout(), 'Full reconciliation');
        // Check for errors
        for (let i = 0; i < results.length; i++) {
            if (results[i].error) {
    ...
    if (navigator.onLine && !_schemaValidated) {
        ...
        validateSchema()
            ...
            if (!result.valid) {
                ...
                syncStatusStore.setStatus('error');
                syncStatusStore.setError(msg, 'Create the required tables in your Supabase project. See stellar-drive README for the required SQL schema.');
            }
        })
            .catch(() => { });
    }
...
export async function repairSyncQueue() {
    ...
    const pendingEntityIds = new Set(pendingItems.map((item) => item.entityId));
    ...
    for (const tableConfig of sortedTables) {
        ...
        for (const record of allLocal) {
            if (!record.id)
                continue;
            if (record.deleted)
                continue; // Don't re-queue tombstones
 ...
 */
function isChildTable(schema, schemaKey) {
    const def = schema[schemaKey];
    if (!def || typeof def !== 'object')
        return false;
    const ownership = def.ownership;
