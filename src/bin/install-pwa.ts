...
.stellar/              # Schema snapshot for migration diffing (gitignored)
...
- **First run**: When no snapshot exists, generates the full initial SQL (CREATE TABLE, RLS, triggers, indexes).
...
> \`SUPABASE_SERVICE_ROLE_KEY\` is optional. Without it, types still auto-generate but Supabase migrations are skipped. The migration SQL is printed to the console so you can apply it manually.
...
4. **Updates the snapshot** for the next diff

This runs on every save in dev mode (500ms debounce) and once during production builds.
