...
self.addEventListener('install', (event: ExtendableEvent) => {
  ...
  event.waitUntil(
    caches.open(SHELL_CACHE).then(async (cache: Cache) => {
      ...
      await Promise.all(
        cacheNames
          .filter((name) => {
            ...
            if (name.startsWith('__SW_PREFIX__-shell-') && name !== SHELL_CACHE) return true;
            /* Delete legacy shared cache (one-time migration) */
            if (name === '__SW_PREFIX__-cache-v1') return true;
            return false;
          })
          .map((name) => {
...
async function backgroundPrecache(): Promise<void> {
  try {
    ...
    console.log(
      `[SW] Caching ${uncached.length} new assets (${assets.length - uncached.length} already cached)`
    );
...
async function cleanupOldAssets(): Promise<void> {
  try {
    ...
    const manifest = await manifestResponse.json();
    const currentAssets = new Set<string>(manifest.assets || []);
...
async function getCacheStatus(): Promise<{
  ...
  try {
    ...
    const manifestResponse =
      (await shellCache.match('/asset-manifest.json')) ||
    ...
    return { cached: 0, total: 0, ready: false, error: (e as Error).message };
  }
}
