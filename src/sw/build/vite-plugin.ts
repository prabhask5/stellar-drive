 ...
 */
async function discoverExistingTables(
  prefix: string,
  opts: ResolvedSchemaConfig,
  root: string
): Promise<string[]> {
  ...
  try {
    ...
    return rows
      ...
      .filter((name: string) => name.startsWith(tablePrefix))
      .map((name: string) => name.slice(tablePrefix.length));
  } catch (err) {
