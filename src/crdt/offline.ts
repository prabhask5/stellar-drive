...
function getOfflineDbName(): string {
  return `${getEngineConfig().prefix}_offline_crdt`;
}

function getObjectStoreName(): string {
  return 'cached_docs';
}
 ...
 */
export async function cacheCrdtForOffline(docId: string): Promise<void> {
  try {
    ...
    if (doc) {
      ...
      if (!data?.yjs_state) {
        ...
        // Decode base64
        const binary = atob(data.yjs_state as string);
 ...
 * @param docId - Document/note ID
 */
export async function removeCrdtOfflineCache(docId: string): Promise<void> {
  try {
 ...
 */
export async function isCrdtCachedOffline(docId: string): Promise<boolean> {
  try {
    const db = await openOfflineDb();
