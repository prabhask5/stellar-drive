...
const collaboratorsByDocument: Map<string, Map<string, UserPresenceState>> = new Map();

/** Change listeners per document. */
const changeListeners: Map<string, Set<(collaborators: UserPresenceState[]) => void>> = new Map();
 ...
 * @internal
 */
export function handlePresenceJoin(documentId: string, presence: UserPresenceState): void {
  ...
  debugLog(
    `[CRDT] Document ${documentId}: ${presence.name} joined (userId=${presence.userId}, deviceId=${presence.deviceId})`
  );
 ...
 * @internal
 */
export function handlePresenceLeave(documentId: string, userId: string, deviceId: string): void {
  const collaborators = collaboratorsByDocument.get(documentId);
  if (!collaborators) return;
 ...
 * });
 */
export function updateCursor(documentId: string, cursor: unknown, selection?: unknown): void {
  ...
  if (now - lastUpdate < config.cursorDebounceMs) {
    debugLog(
      `[CRDT] Document ${documentId}: cursor update throttled (${now - lastUpdate}ms < ${config.cursorDebounceMs}ms)`
    );
    return;
  }
 ...
 */
export function getCollaborators(documentId: string): UserPresenceState[] {
  const collaborators = collaboratorsByDocument.get(documentId);
  if (!collaborators) return [];

  const localDeviceId = getDeviceId();
  const result: UserPresenceState[] = [];
 ...
 */
function notifyListeners(documentId: string): void {
