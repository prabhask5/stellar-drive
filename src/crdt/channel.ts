...
import type {
  ...
  BroadcastSyncStep1Message,
  BroadcastSyncStep2Message,
  BroadcastChunkMessage,
  CRDTConnectionState
} from './types';
...
export class CRDTChannel {
  ...
  private onConnectionStateChange: ((state: CRDTConnectionState) => void) | null = null;

  /** Initial presence info for Supabase Presence tracking. */
  private presenceInfo: { name: string; avatarUrl?: string } | null = null;
  ...
  // --- Sync protocol state ---
  private syncResolvers: Map<string, () => void> = new Map();

  constructor(
    documentId: string,
    doc: Y.Doc,
    onConnectionStateChange?: (state: CRDTConnectionState) => void
  ) {
   ...
   */
  setPresenceInfo(info: { name: string; avatarUrl?: string }): void {
  ...
  async join(): Promise<void> {
    ...
    this.channel.on('presence', { event: 'join' }, ({ newPresences }) => {
      for (const presence of newPresences) {
        const state = presence as unknown as UserPresenceState;
  ...
  broadcastUpdate(update: Uint8Array): void {
    if (this.destroyed || this._connectionState !== 'connected') return;
    ...
    return new Promise<boolean>((resolve) => {
      ...
      setTimeout(() => {
        ...
        if (!resolved) {
          resolved = true;
          debugLog(
            `[CRDT] Document ${this.documentId}: no peers responded within ${config.syncPeerTimeoutMs}ms, fetching from Supabase`
          );
          resolve(false);
        }
  ...
  private handleBroadcastMessage(message: BroadcastMessage): void {
    ...
    if (message.deviceId === this.deviceId) {
      debugLog(
        `[CRDT] Document ${this.documentId}: skipped own-device echo (deviceId=${this.deviceId})`
      );
      return;
    }
   ...
   */
  private handleRemoteUpdate(message: BroadcastUpdateMessage): void {
   ...
   */
  private handleSyncStep1(message: BroadcastSyncStep1Message): void {
    ...
    if (update.byteLength > 0) {
      debugLog(
        `[CRDT] Document ${this.documentId}: sync-step-2 sent to ${message.deviceId} (${update.byteLength} bytes)`
      );
      this.sendMessage({
        type: 'sync-step-2',
   ...
   */
  private handleSyncStep2(message: BroadcastSyncStep2Message): void {
  ...
  private flushUpdates(): void {
    ...
    debugLog(
      `[CRDT] Document ${this.documentId}: ${updates.length} updates buffered (${merged.byteLength} bytes), broadcasting`
    );
  ...
  private sendSyncStep1(): void {
    ...
    debugLog(
      `[CRDT] Document ${this.documentId}: sync-step-1 sent (stateVector ${stateVector.byteLength} bytes)`
    );
    this.sendMessage({
      type: 'sync-step-1',
   ...
   */
  private sendMessage(message: BroadcastMessage): void {
   ...
   */
  private sendChunked(base64Data: string): void {
    const config = getCRDTConfig();
    ...
    debugWarn(
      `[CRDT] Document ${this.documentId}: chunking broadcast payload (${base64Data.length} bytes > ${config.maxBroadcastPayloadBytes} bytes, ${totalChunks} chunks)`
    );
  ...
  private handleDisconnect(): void {
    ...
    if (this.reconnectAttempts >= config.maxReconnectAttempts) {
      debugWarn(
        `[CRDT] Channel ${this.channelName} max reconnect attempts reached (${config.maxReconnectAttempts})`
      );
      return;
    }
    ...
    debugLog(
      `[CRDT] Channel ${this.channelName} reconnecting (attempt ${this.reconnectAttempts}/${config.maxReconnectAttempts}, delay ${delay}ms)`
    );
  ...
  private trackPresence(): void {
    if (!this.channel || !this.presenceInfo || this._connectionState !== 'connected') return;
   ...
   */
  private setConnectionState(state: CRDTConnectionState): void {
