...
function generateTableSQL(
  ...
  for (const colDef of columnDefs) {
    ...
    if (baseType) {
      lines.push(
        `do $$ begin alter table ${tableName} alter column ${colName} type ${baseType} using ${colName}::${baseType}; exception when others then null; end $$;`
      );
    }
    ...
    if (!colTypeLower.includes('not null')) {
      ...
      lines.push(
        `do $$ begin alter table ${tableName} alter column ${colName} set not null; exception when others then null; end $$;`
      );
    }
  ...
  if (isChildTable) {
    ...
    lines.push(
      `create policy "Owner can view ${tableName}" on ${tableName} for select using (${check});`
    );
    lines.push(
      `create policy "Owner can create ${tableName}" on ${tableName} for insert with check (${check});`
    );
    lines.push(
      `create policy "Owner can update ${tableName}" on ${tableName} for update using (${check});`
    );
    lines.push(
      `create policy "Owner can delete ${tableName}" on ${tableName} for delete using (${check});`
    );
  } else {
    lines.push(
  ...
  if (config.uniqueConstraints && config.uniqueConstraints.length > 0) {
    for (const constraint of config.uniqueConstraints) {
      ...
      lines.push(
        `create unique index if not exists ${name} on ${tableName}(${cols})${whereClause};`
      );
    }
  }
  lines.push('');
...
export function generateSupabaseSQL(
  ...
  if (includeDeviceVerification) {
    ...
    parts.push(
      'create index if not exists idx_trusted_devices_app_prefix on trusted_devices(app_prefix);'
    );
    parts.push('');
    parts.push(
