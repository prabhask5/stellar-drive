 ...
 */
export async function bulkRemoveSyncItems(ids: number[]): Promise<void> {
  if (ids.length === 0) return;
  const db = getDb();
  await db.table('syncQueue').bulkDelete(ids);
}
...
export async function exitBatchMode(): Promise<void> {
  batchMode = false;
