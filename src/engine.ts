...
async function pullRemoteChanges(minCursor?: string): Promise<{ bytes: number; records: number }> {
  ...
  const results = await withTimeout(Promise.all(
    config.tables.map(table =>
      supabase.from(table.supabaseName).select(table.columns).gt('updated_at', lastSync)
    )
    ...
    async () => {
      for (let i = 0; i < config.tables.length; i++) {
        ...
        await applyRemoteWithConflictResolution(
          tableNames[i],
          data,
          db.table(config.tables[i].dexieTable)
        );
      }
    }
  );
...
export async function debugTombstones(options?: {
  ...
  for (const tableConfig of config.tables) {
    ...
    if (allDeleted.length > 0) {
      debugLog(
        `  ${tableConfig.dexieTable}: ${allDeleted.length} tombstones (${eligible.length} eligible for cleanup)`
      );
...
function initDebugSyncUtilities(): void {
  if (typeof window === 'undefined' || !isDebugMode()) return;
  ...
  (window as any)[`${prefix}Sync`] = {
    ...
    testRealtime: async () => {
      const config = getEngineConfig();
      const firstTable = config.tables[0]?.supabaseName;
      if (!firstTable) return 'No tables configured';
      ...
      const channel = supabase
        ...
        .on(
          'postgres_changes',
          { event: '*', schema: 'public', table: firstTable },
          (payload) => {
