 ...
 */
function ensureSystemFieldDefaults(payload: Record<string, unknown>): Record<string, unknown> {
 ...
 */
function filterPayloadToSchema(
  tableName: string,
...
async function acquireSyncLock(): Promise<boolean> {
  ...
  if (lockPromise !== null) {
    if (lockAcquiredAt && Date.now() - lockAcquiredAt > SYNC_LOCK_TIMEOUT_MS) {
      debugWarn(
        `[SYNC] Force-releasing stale sync lock (held for ${Math.round((Date.now() - lockAcquiredAt) / 1000)}s)`
      );
      releaseSyncLock();
    } else {
      debugLog(
...
async function pushPendingOps(): Promise<PushStats> {
  ...
  let progressMonitor: ReturnType<typeof setInterval> | null = null;
  ...
  try {
      ...
      if (batchableItems.length > 0) {
        ...
        for (const [tableName, items] of itemsByTable) {
          ...
          for (const item of items) {
            ...
            const rawPayload: Record<string, unknown> = { ...localEntity, device_id: deviceId };
            ensureSystemFieldDefaults(rawPayload);
      ...
      for (const item of individualItems) {
        try {
          if (item.id) {
            ...
            if (!stillQueued) {
              debugLog(
                `[SYNC] Skipping purged item: ${item.operationType} ${item.table}/${item.entityId}`
              );
              continue;
            }
          }
    ...
    if (progressMonitor) {
      ...
      try {
        ...
        if (delta !== 0) syncStatusStore.advanceProgress(delta);
      } catch (err) {
    ...
    case 'increment': {
      ...
      if (!data) {
        ...
        if (localInc) {
          ...
          const rawInsertPayload: Record<string, unknown> = { ...localInc, device_id: deviceId };
    ...
    case 'set': {
      ...
      if (!data) {
        ...
        if (localEntity) {
          ...
          const rawSetPayload: Record<string, unknown> = { ...localEntity, device_id: deviceId };
  ...
  try {
    for (const tableConfig of config.tables) {
      ...
      if (error) {
        ...
        debugLog(
          `[Tombstone] Cleaned ${data.length} old records from server ${tableConfig.supabaseName}`
        );
...
export async function startSyncEngine(): Promise<void> {
  ...
  isOnline.onDisconnect(async () => {
    ...
    try {
      ...
      if (
        ...
        debugWarn(
          '[Engine] Cached credentials do not match current user - skipping offline session creation'
        );
        return;
      }
    ...
    try {
      ...
      if (user) {
        ...
        if (timeSinceLastSync < getOnlineReconnectCooldownMs()) {
          debugLog(
            `[SYNC] Skipping reconnect sync (last sync ${Math.round(timeSinceLastSync / 1000)}s ago)`
          );
        } else {
  ...
  watchdogInterval = setInterval(() => {
    ...
    if (lockAcquiredAt && Date.now() - lockAcquiredAt > SYNC_LOCK_TIMEOUT_MS) {
      debugWarn(
        `[SYNC] Watchdog: sync lock stuck for ${Math.round((Date.now() - lockAcquiredAt) / 1000)}s - force-releasing and retrying`
      );
      releaseSyncLock();
      syncStatusStore.setStatus('idle');
