...
/**
 * Internal state object for the realtime manager singleton.
 * All mutable connection state is consolidated here for clarity.
 *
 * **Design rationale:** Grouping all mutable state into a single object makes it
 * easy to audit what can change at runtime, simplifies debugging (you can
 * inspect one object), and makes teardown/reset logic straightforward
 * (reset each field to its initial value).
 */
interface RealtimeManagerState {
  /** The active Supabase Realtime channel, or null when disconnected. */
  channel: RealtimeChannel | null;

  /** Current connection lifecycle state. @see {@link RealtimeConnectionState} */
  connectionState: RealtimeConnectionState;

  /** The authenticated user's ID; used to create a unique channel name. */
  userId: string | null;

  /** This device's unique identifier; used for echo suppression. @see {@link getDeviceId} */
  deviceId: string;

  /** Human-readable description of the most recent connection error, if any. */
  lastError: string | null;

  /** How many consecutive reconnection attempts have been made since the last success. */
  reconnectAttempts: number;

  /** Handle for the pending reconnection timer, so it can be cancelled on teardown. */
...
const pendingNotifications = new Map<string, Set<string>>();
...
function flushPendingNotifications(): void {
  ...
  debugLog(
    `[Realtime] Flushing batched notifications: ${[...snapshot.entries()].map(([t, ids]) => `${t}(${ids.size})`).join(', ')}`
  );
 ...
 */
export async function suspendRealtimeForBatchPush(): Promise<void> {
 ...
 */
export async function resumeRealtimeAfterBatchPush(): Promise<void> {
  if (!batchSuspended) return;
